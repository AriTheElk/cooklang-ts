    ...
    parse(source: string): ParseResult {
        ...
        stepLoop: for (let i = 0; i < lines.length; i++) {
            ...
            for (let match of line.matchAll(tokens)) {
                ...
                // temperature
                if (groups.temperatureQuantity) {
                    step.push({
                        type: "temperature",
                        name: groups.temperatureName,
                        quantity: parseQuantity(groups.temperatureQuantity) ?? 0,
                        units: parseUnits(groups.temperatureUnits) ?? this.defaultUnits,
                    });
                }
