...
const multiwordIngredient =
    /@(?<mIngredientName>[^@#~[]+?)\{(?<mIngredientQuantity>[^]*?)(?:%(?<mIngredientUnits>[^}]+?))?\}/;
...
const temperature = /!(?<temperatureName>.*?)(?:\{(?<temperatureQuantity>.*?)(?:%(?<temperatureUnits>.+?))?\})/;
...
export const tokens = new RegExp(
    [metadata, multiwordIngredient, singleWordIngredient, multiwordCookware, singleWordCookware, timer, temperature]
        .map((r) => r.source)
        .join("|"),
    "gu"
);
